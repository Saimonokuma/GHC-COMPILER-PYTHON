...
FIX v5: Fixed LD_LIBRARY_PATH propagation to ghc-pkg recache.
       ...
       Thread sterilized environment through _resolve_runtime_paths
       → _rebuild_package_cache → _ghc_pkg_recache.
...
def _try_resolve_binary(name: str) -> Optional[str]:
    ...
    return next(
        (str(p) for p in candidates if p.exists()), shutil.which(binary_name)
    )
...
def _sterilize_environment() -> dict:
    ...
    env = {
        k: v for k, v in os.environ.items() if k not in HASKELL_POLLUTION_VARS
    }
...
class BaseResource:
    ...
    def locate(
        cls, base: str = sys.prefix, version: str = GHC_VERSION
    ) -> List[Path]:
    ...
    @classmethod
    def patch_build_time(
        cls, path: Path, version: str, placeholder: str
    ) -> int:
    ...
    def patch_build_time(
        ...
        for conf_file in path.glob("*.conf"):
            try:
                ...
                # 🧪 Alchemist: Combine regex patterns into a single pass using alternation
                pattern = re.compile(
                    r"(dynamic-library-dirs:\s*|library-dirs:\s*|include-dirs:\s*)"
                    r"/[^\s]+|/ghc-prefix/lib/ghc-"
                    + re.escape(version)
                    + r"|/ghc-prefix"
                )
                ...
                def repl(m: re.Match) -> str:
                    g1 = m.group(1)
                    if g1:
                        inc = "/include" if "include" in g1 else ""
                        return f"{g1}{placeholder}/lib/ghc-{version}{inc}"
                    return (
                        placeholder
                ...
                sys.stderr.write(
                    f"WARNING: Failed to patch {conf_file}: {e}\n"
                )
...
def _resolve_runtime_paths(env: dict) -> None:
    ...
    for target in set(
        targets
    ):  # 🧪 Alchemist: Deduplicate targets in a single pass
        ...
        try:
            ...
            if content_to_write is not None:
                ...
                if (
                    b" " in prefix_clean_bytes
                    and b"\0" not in content_to_write
                ):
                ...
                with target_path.open("wb") as out:
                    out.write(
                        content_to_write.replace(
                            b"@GHC_PREFIX@", prefix_clean_bytes
                        )
                    )
...
def _ghc_pkg_recache(pkg_db_dir: str, env: dict) -> None:
    ...
    try:
        ...
        sys.stderr.write(
            f"WARNING: ghc-pkg recache failed for {pkg_db_dir}: {e}\n"
        )


def _execute_tool(
    tool_name: str, extra_args: Optional[List[str]] = None
) -> NoReturn:
