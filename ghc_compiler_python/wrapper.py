...
def _sterilize_environment() -> dict:
    ...
    # 🔨 Forge: Added stable tempdir fallback before mkdtemp to prevent leaking directories when os.execve replaces process
    safe_home = (
    ...
    # 🔨 Forge: Reverted to if/elif to maintain Python 3.8+ compatibility (match is 3.10+)
...
def _ghc_pkg_recache(pkg_db_dir: str, env: dict) -> None:
    ...
    try:
        ...
        # 🔨 Forge: Reverted to dictionary unpacking to maintain Python 3.8+ compatibility (| is 3.9+)
        subprocess.run(
