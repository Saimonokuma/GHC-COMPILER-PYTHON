...
class BaseResource:
    ...
    def locate(cls, base: str = sys.prefix, version: str = GHC_VERSION) -> List[Path]:
        ...
        if match := next((c for c in candidates if (c.is_dir() if cls.is_dir else c.is_file()) and cls.validate(c)), None):
            return [match]
...
def _resolve_runtime_paths(env: dict) -> None:
    ...
    # 🧪 Alchemist: Set comprehension condenses nested loops for dynamic target extraction
    targets = {
