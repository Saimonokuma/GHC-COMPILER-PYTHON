...
def _find_platform_lib_subdir() -> str:
    ...
    try:
        return next((str(c) for c in ghc_lib_dir.iterdir() if _safe_is_dir(c) and c.name.endswith(f"-ghc-{GHC_VERSION}")), "")
    except OSError:
        return ""
...
class BinWrappersResource(BaseResource):
    ...
    def extract_targets(cls, path: Path) -> List[str]:
        try:
            return [
                ...
                if _safe_is_file(f) and not f.is_symlink() and not f.name.endswith(".exe") and _is_text_file(f)
            ]
        except OSError:
            return []
    ...
    def patch_build_time(cls, path: Path, version: str, placeholder: str) -> int:
        ...
        for script in path.iterdir():
            if not _safe_is_file(script) or script.is_symlink() or script.name.endswith(".exe") or not _is_text_file(script):
                continue
            try:
