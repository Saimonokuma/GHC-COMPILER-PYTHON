...
class PackageDBResource(BaseResource):
    ...
    def patch_build_time(cls, path: Path, version: str, placeholder: str) -> int:
        ...
        # 🧪 Alchemist: Combine regex patterns into a single pass using alternation
        # Moved outside the loop to avoid O(N) compilations
        pattern = re.compile(
...
class BinWrappersResource(BaseResource):
    ...
    def patch_build_time(cls, path: Path, version: str, placeholder: str) -> int:
        ...
        # 🧪 Alchemist: Move static variables and regex compilation outside the loop
        ...
        # Combine regex patterns into a single pass using alternation
        pattern = re.compile(
...
def _resolve_runtime_paths(env: dict) -> None:
    ...
    # 🧪 Alchemist: Set comprehension directly generates a deduplicated set of targets, avoiding intermediate lists
    targets = {
