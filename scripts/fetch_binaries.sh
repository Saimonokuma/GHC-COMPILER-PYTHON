...
# Copy GHC components to staging
cp -a "build_artifacts/${GHC_EXTRACTED_DIR}/bin/." ghc-bindist/bin/ 2>/dev/null || true
cp -a "build_artifacts/${GHC_EXTRACTED_DIR}/lib" ghc-bindist/ 2>/dev/null || true
cp -a "build_artifacts/${GHC_EXTRACTED_DIR}/share" ghc-bindist/ 2>/dev/null || true

# Copy settings and package.conf.d if they exist
...
find ghc-bindist -type f | head -20
