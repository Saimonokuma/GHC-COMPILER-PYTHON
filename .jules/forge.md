...
### Critical Learnings (YYYY-MM-DD):
- In `ghc_compiler_python/wrapper.py`, `Path.iterdir()` can raise `OSError` (e.g. `PermissionError`) when used on an unreadable directory. Added `try...except OSError:` around `iterdir()` in `_find_platform_lib_subdir` and `BinWrappersResource.patch_build_time` to prevent crashes and return safe fallbacks.
