...
### Critical Learnings (2026-07-03):
- Discovered that Python's `pathlib` methods like `exists()`, `is_dir()`, `is_file()`, `is_symlink()`, `iterdir()`, and `glob()` can throw a `PermissionError` (a subclass of `OSError`) on heavily restricted systems when traversing directories. This can break dynamic discovery (like `_resolve_runtime_paths` and class resources in `wrapper.py`). Replaced raw `pathlib` calls with new internal `_safe_*` wrappers in `ghc_compiler_python/wrapper.py` that catch `OSError` and return a safe default (e.g. `False` or an empty list).
