...
### Critical Learnings (2025-05-26):
- Fixed missing `OSError` catch around `iterdir()` in `ghc_compiler_python/wrapper.py`. Calls to `iterdir()` raise `PermissionError` (a subclass of `OSError`) on unreadable directories. Added `try...except OSError` handling in `_find_platform_lib_subdir` and `BinWrappersResource.patch_build_time` to prevent crashes during path resolution.
- CI failures related to "unable to access... The requested URL returned error: 403" on `actions/checkout` when the repo owner's account is suspended can be mitigated by disabling credential persistence (`persist-credentials: false`), preventing the runner from trying to use a revoked token for subsequent git operations.
