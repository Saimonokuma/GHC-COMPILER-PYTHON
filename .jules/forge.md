...
### Critical Learnings (YYYY-MM-DD):
- Fixed a race condition in `ghc_compiler_python/wrapper.py` (`_resolve_runtime_paths`) where multiple processes could corrupt GHC internal text files or binaries by attempting to write in-place simultaneously. Implemented atomic file replacements using uniquely named `.tmp` files (`uuid.uuid4().hex`) and `os.replace`. Also added `try...finally` cleanup blocks to prevent `.tmp` file leakage in the event of write errors (such as disk full or `PermissionError`).
