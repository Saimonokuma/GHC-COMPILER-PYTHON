...
### Critical Learnings (YYYY-MM-DD):
- Restored compatibility with Python 3.8 and 3.9 in `ghc_compiler_python/wrapper.py` by removing Python 3.10+ `match`/`case` pattern matching and the Python 3.9+ dictionary merge operator `|`, replacing them with standard `if`/`elif` blocks and dictionary unpacking (`**`).
- Fixed a silent temp directory leak. Because `os.execve` replaces the Python process, temporary directories generated by `tempfile.mkdtemp()` are never cleaned up by Python's `atexit` handlers. Added a stable temp directory fallback using `tempfile.gettempdir()` before `mkdtemp()` to prevent infinite accumulation of temp folders on systems where `os.execve` is used.
