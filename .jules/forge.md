...
### Critical Learnings (YYYY-MM-DD):
- Fixed missing `OSError` catch blocks around `Path.iterdir()` and `Path.glob()` in `ghc_compiler_python/wrapper.py`. `_find_platform_lib_subdir`, `BinWrappersResource.patch_build_time`, and `PackageDBResource.patch_build_time` could crash with `PermissionError` when operating on directories lacking read permissions. They now gracefully handle these errors.
